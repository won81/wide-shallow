...
st.write('### [데이터시각화] 연도별 서울 아파트 가격 변동 그래프')
st.write('check out this [link](https://wide-shallow.tistory.com/entry/%EB%8D%B0%EC%9D%B4%ED%84%B0%EC%8B%9C%EA%B0%81%ED%99%94-%EC%97%B0%EB%8F%84%EB%B3%84-%EC%84%9C%EC%9A%B8-%EC%95%84%ED%8C%8C%ED%8A%B8-%EA%B0%80%EA%B2%A9-%EB%B3%80%EB%8F%99-%EA%B7%B8%EB%9E%98%ED%94%84)')
...
with st.container():
    if st.session_state.deals:
        ...
        areas['면적'] = sorted(list(set(df[(df['법정동'] == beopjeongdong) & (df['아파트'] == apartment)]['전용면적'].values.tolist())))
        for i in range(len(areas['면적'])):
            areas['Selected'].append(True)
        ...
        df_selected_except_area = df[(df['법정동'] == beopjeongdong) & (df['아파트'] == apartment)]
        df_selected = pd.DataFrame()
        for area, selected in zip(areas['면적'], areas['Selected']):
            if selected == True:
                df_selected = pd.concat([df_selected, df_selected_except_area[df_selected_except_area['전용면적'] == area]])
